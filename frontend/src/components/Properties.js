...
import fetchProperties from "../utils/custom-hooks/Fetcher";
...
export default () => {
  const [{ data, isLoading, isError, error }] = fetchProperties();
      ...
      {Object.keys(sortedPropertiesObj).map(property_key => (
        <div className="type-devider">
          <Divider style={{ padding: 0, margin: 0 }} orientation="left">
            <FontAwesomeIcon icon={faFire} size="sm" color="#e25822"/> Popular in {property_key}
          </Divider>
          <ul className="row pt-5">
            {sortedPropertiesObj[property_key].slice(0, 6).map(property => (
